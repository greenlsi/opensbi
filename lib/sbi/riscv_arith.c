// the implementation of floating point operation to fix build issue with CVA6 32 Bit that does not support division and module operation
// CVA6 32 bit only support F extension, while glibc does not support ABI ilp32f and soft floating point so no floating point operation is allowed
// for example, in sbi_console.c and sbi_timer.c there is division and module operation which cause build failed in 32 bit 
// this code is taken from https://github.com/glitchub/arith64 
